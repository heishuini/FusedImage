...
# 在训练时随机“跳过”（即丢弃）某些网络层的前向计算路径，而在测试时保留完整路径
# 正则化和训练加速
...
class Attention(nn.Module):
    def __init__(self,
        ...
        # nn.Linear, 如果输入是3D或更高维张量，会自动作用在最后一个维度，其他不变
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        # 线性 Wx+b
        x = self.fc1(x)
        # 非线性激活，增加非线性表达能力，能够拟合更多和表达更复杂的函数
        x = self.act(x)
        x = self.drop(x)
        x = self.fc2(x)
        x = self.drop(x)
...
def _init_vit_weights(m):
    ...
    if isinstance(m, nn.Linear):
        # 截断正态分布初始化，控制初始化值的范围，使其更加集中，确保训练初期的稳定性
...
class VisionTransformer(nn.Module):
    def __init__(self, image_size=224, patch_size=16, in_c=3, num_classes=1000,
        ...
        self.num_features = self.embed_dim = embed_dim # patch_size*patch_size*in_c
        ...
        # representation:全连接层，logits是全连接层的输出*权重
        # todo: need understand
        # 预分类头，将embed_dim转到预分类的维度
...
if __name__ == "__main__":
    ...
    net = VisionTransformer(image_size=512, patch_size=16, in_c=3, num_classes=8)

    result = net(image)

    print(result.shape) # (B, num_classes)
    print(result.data)
