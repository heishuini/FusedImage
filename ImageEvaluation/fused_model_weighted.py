...
CLIPIQA_result_path = "../../data/DIV2K/crop_1024/x2/weighted"

def evaluate_folders(folder_paths, output_csv="../../data/DIV2K/crop_1024/x2/weighted/clip_iqa_results.csv"):
    """
    评估多个文件夹中的图像质量
    参数:
        folder_paths: 包含8个文件夹路径的字典
        output_csv: 结果保存路径
    """
    # 初始化CLIPIQA评估器
    ...
    for i in tqdm(range(num_images), desc="Processing images"):
        img_result = {"image_id": i+1}  # 使用1-based索引
        ...
        for method in folder_paths.keys():
            ...
            # 加载图像并评估
            try:
                # img_tensor = load_image_as_tensor(img_path).to(device)
                im_in = util_image.imread(img_path, chn='rgb', dtype='float32')
                im_in_tensor = util_image.img2tensor(im_in).to(device)

                with torch.no_grad():
                    score = clipiqa_metric(im_in_tensor).item()
                img_result[method] = score
...
def weighted_fusion(images, scores, image_id):
    ...
    util_image.imwrite(normalized_img,os.path.join('../../data/DIV2K/crop_1024/x2/weighted/resultImage',f'result_{image_id:03d}.png'))
...
if __name__ == "__main__":
    ...
    # CLIPIQA得分计算
    results_df = evaluate_folders(folder_paths)
    
    # 根据得分进行融合
    score_df = pd.read_csv('../../data/DIV2K/crop_1024/x2/weighted/clip_iqa_results.csv')
    # image_id,universal,deblur,dehaze,denoise,derain,deraindrop,lowlight,sr
    # print(score_df['image_id'])

    select_columns = score_df.columns[1:33]
    scores_df = score_df[select_columns][0:100] # 最后是avg要去掉
    print(scores_df) # 100rows，8columns

    # print(list(scores_df.iloc[0])) # 获取第一行
    score_list = scores_df.values.tolist()
    # print(score_list)
    ...
    for i in tqdm(range(num_images), desc="Processing images"):
        ...
        # 简单的加权融合
        weighted_fusion(im_in_list,score_list[i],image_id=i)
        # 
