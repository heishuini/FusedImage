...
# 方法分组
methods = ['Pixelwise', 'Blockwise']

# 每种方法下，分别是 (+版本, 原版)
metric1 = [[69.382, 69.390], [64.783, 69.342]]
metric2 = [[24.896, 24.896], [23.905, 24.809]]
metric3 = [[0.248, 0.247], [0.282, 0.247]]
metric4 = [[0.657, 0.657], [0.616, 0.657]]

metrics = [metric1, metric2, metric3, metric4]
metric_names = ['MUSIQ', 'PSNR', 'LPIPS', 'SSIM']
...
for i in range(4):
    axs[i].bar(x - bar_width/2, [m[0] for m in metrics[i]], width=bar_width, label='MSE+LPIPS', color='skyblue')
    axs[i].bar(x + bar_width/2, [m[1] for m in metrics[i]], width=bar_width, label='MSE', color='salmon')
    axs[i].set_xticks(x)
...
# plt.suptitle('Comparison of Methods (+ vs Original)', fontsize=16)
