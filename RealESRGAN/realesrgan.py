...
from basicsr.utils import DiffJPEG, USMSharp, img2tensor, tensor2img, imfrombytes, FileClient
...
def load_images_degradation(input_path,output_path,file_client, degradation_model,exts=('*.png', '*.jpg', '*.jpeg')):

    for idx,path in enumerate(input_path):
        # 得是字节流读取
        img_bytes = file_client.get(path, 'gt')
        img_gt = imfrombytes(img_bytes, float32=True)

        img_gt, img_lq = degradation_model.degrade_process(img_gt)
        print(f"{img_gt.shape}, {img_lq.shape}")

        img_lq = img_lq[0]
        img_lq = img_lq.permute(1, 2, 0).mul(255).byte().cpu().numpy()
        Image.fromarray(img_lq).save(f'{output_path}/lq_{idx+1:04d}.png')

        # img_gt = img_gt[0]
        # img_gt = img_gt.permute(1, 2, 0).mul(255).byte().cpu().numpy()
        # Image.fromarray(img_gt).save(f'{output_path}/gt/gt_{idx+1:03d}.png')
...
with open(meta_info) as fin:
    ...
    paths = [os.path.join(input_folder, v) for v in paths]

io_backend_opt = {'type': 'disk'}
file_client = FileClient(io_backend_opt.pop('type'), **io_backend_opt)

output_path = "../data/DIV2K/crop_512/lq_severe" # change this
degradation_model = RealESRGAN_degradation(opt_name='severe_degrade.yml')

load_images_degradation(paths, output_path, file_client, degradation_model)